...
`;
      fs.writeFile(domainEntity, domainContent, (error) => {
        ...
        console.log(
          `El archivo '${result.name}' se creó dentro del directorio '${domain}' correctamente.`
        );
      });
      fs.writeFile(domainInterface, domainInterfaceContent, (error) => {
        ...
        console.log(
          `El archivo 'I${result.name}Repository' se creó dentro del directorio '${domain}' correctamente.`
        );
      });
    });
    fs.mkdir(infrastructure, { recursive: true }, (error) => {
      const infrastructureRepository = path.join(
        infrastructure,
        `${result.name}Repository.ts`
      );
...
`;
      fs.writeFile(infrastructureRepository, infrastructureContent, (error) => {
        ...
        console.log(
          `El archivo '${result.name}Repository' se creó dentro del directorio '${infrastructure}' correctamente.`
        );
      });
      ...
      fs.mkdir(controller, { recursive: true }, (error) => {
        ...
        const controllerCreate = path.join(
          controller,
          `Create${result.name}Controller.ts`
        );
  ...
  constructor( private readonly create: Create ) {}
   async run(){
     try {
       ...
       return result;
        ...
        fs.writeFile(controllerCreate, controllerContent, (error) => {
          ...
          console.log(
            `El archivo 'Create${result.name}Controller' se creó dentro del directorio '${controller}' correctamente.`
          );
        });
      });
    });
    ...
    fs.mkdir(application, { recursive: true }, (error) => {
      ...
      const createContent = `
import type { I${result.name}Repository } from '../domain/I${
        result.name
      }Repository.js';
export class Create {
  constructor( private readonly ${result.name.toLowerCase()}Repository: I${
        result.name
      }Repository  ) {}
  async run(){
...
`;
      ...
      fs.writeFile(createUsecase, createContent, (error) => {
        ...
        console.log(
          `El archivo 'create' se creó dentro del directorio '${application}' correctamente.`
        );
      });
    });
  });
